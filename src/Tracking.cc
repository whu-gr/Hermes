...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL)), mBlockNum(64)
{
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO || mSensor==System::RGBD ){
        mbf = settings->bf();
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO || mSensor==System::RGBD || mSensor==System::RGBD)
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    equalizeHist(imGrayRight, imGrayRight);

    CalculateBlockEntropy(2.0);
...
void Tracking::Track()
{
    ...
    // 地图更新时加锁，保证地图不会发生变化
    // 加锁并不会阻碍地图实时更新，因为SLAM主要耗时在前端，此时地图没有上锁，有时间更
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        if(!mbOnlyTracking){
            ...
            if(mState==OK){
                ...
                // 速度为空或者刚刚完成重定位，跟踪参考关键帧，否则恒速模型跟踪
                if((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2){
                    Verbose::PrintMess("TRACK: Track with respect to the reference KF ", Verbose::VERBOSITY_DEBUG);
                    // BoW在参考帧中找到当前帧特征点的匹配点，优化每个点的3D重投影误差来得到位姿
                    ...
                    if(!bOK){
                        cout << "TrackMotionModel Failed, try to TrackRef" << endl;
                        bOK = TrackReferenceKeyFrame();
                    }
                }
        ...
        // 前面只是跟踪一帧得到的初始位姿，这里搜索局部关键帧和局部地图点，和当前帧进行投影匹配，得到更多MP优化位姿
        if(!mbOnlyTracking)
        {
            if(bOK)
                bOK = TrackLocalMap();
        ...
        // 跟踪成功或者丢失之后才考虑生成关键帧的问题
        if(bOK || mState==RECENTLY_LOST)
        {
            // Update motion model
            if(mLastFrame.isSet() && mCurrentFrame.isSet())
            {
                // mVelocity = Tcl = Tcw*Twl，表示上一帧到当前帧的变换，Twl = LastTwc
            ...
            // 当前帧所有操作完成之后，这个地方清除临时地图点
            // std::cout << mlpTemporalPoints.size() << std::endl;
...
{
    // 遍历关键点，获取其对应的地图点（包括临时地图点）
    for(int i =0; i<mLastFrame.N; i++)
    {
        MapPoint* pMP = mLastFrame.mvpMapPoints[i];
        if(pMP)
        {
            // 用新的替换该点的MapPoint替换原来的MapPoint，在replace函数中利用mpReplaced判定
...
{
    // 这个地方没有使用UpdateLastFrame()，因为只有刚刚初始化或者恒速跟踪失败时才会调用本函数
...
// 这个函数的作用是更新上一帧的位姿，对于双目和RGBD，还在上一帧中生成临时地图点
void Tracking::UpdateLastFrame()
{
    // Update pose according to reference keyframe
    //上一普通帧的参考关键帧，因为位姿更准一些
    KeyFrame* pRef = mLastFrame.mpReferenceKF;
    // ref_KF到lastFrame的位姿变换
    Sophus::SE3f Tlr = mlRelativeFramePoses.back();
    // 计算上一帧在世界坐标系下的坐标 Tlw = Tlr * Trw
    ...
    // Create "visual odometry" MapPoints 生成临时地图点
    ...
    {
        int i = vDepthIdx[j].second; // 上一帧中具有有效深度值点的id
        bool bCreateNew = false;

        // 如果这个点在上一帧中没有对应的地图点，或者创建之后就没有观测到，就生成一个临时地图点
        MapPoint* pMP = mLastFrame.mvpMapPoints[i];
        ...
        if(bCreateNew)
        {
            ...
            // 把这个点反投影到世界坐标系中得到3D点 x3D
            // UnprojectStereo()通过索引获得u,v像素坐标，用公式计算得到3D点x,y分量，新建一个MapPoint对象
            ...
            // 判断临时地图点对应的特征点和正常地图点对应的特征点是不是太近
            // 临时地图点不能和现有的地图点太近

            // 括号里面依次是：世界坐标系下坐标、当前跟踪的地图、存在这个特征点的帧（也就是上一帧）、特征点ID
            MapPoint* pNewMP = new MapPoint(x3D,mpAtlas->GetCurrentMap(),&mLastFrame,i);

            // cout << "Create New TMP by MM" << endl;

            float thEntropy = 2.0;
            if(CheckMapPointEntropy(thEntropy, i) == 0)
                pNewMP->isInLowEntropy = true;
            
            // 加入上一帧的地图点中
            mLastFrame.mvpMapPoints[i]=pNewMP; // 上一帧的地图点创建了多少？为什么可以直接加

            // 标记为临时添加的地图点，之后在CreateNewKeyFrame会完全删除
        ...
        {
            // 记录vDepthIdx中不需要创建地图点的个数
            nPoints++;
        }
        // 地图点质量不好，就停止创建地图点，条件为：
        // 1. 当前点的深度超过了设定的深度阈值（40倍baseline）
        // 2. nPoints已经超过100个点，说明距离已经比较远了（因为容器sort过了）
        if(vDepthIdx[j].first>mThDepth && nPoints>100)
            break;
    }
    // std::cout << "num of temporalPoints: " << nPoints << std::endl;
}

// 用上一普通帧跟踪当前普通帧
// 根据恒速模型设定当前帧初始位姿，用投影的方式在参考帧中找到当前帧特征点的匹配点，优化每个特征点对应的3D点误差即可得到位姿
bool Tracking::TrackWithMotionModel()
{
    // std::cout << "tracking motion model!" << std::endl;
    ORBmatcher matcher(0.9,true);
    ...
    // 有问题，应该是相对位姿，这个地方求的是绝对位姿，看一下相对位姿在哪里，会一直增加，这个地方也不是两帧之间的，看看怎么改
    // mVelocity = Tcl = Tcw*Twl，表示上一帧到当前帧的变换，Twl = LastTwc
    Eigen::Matrix<float,3,3> Rcl = mVelocity.rotationMatrix();
    Eigen::Matrix<float,3,1> tcl = mVelocity.translation();
    Eigen::AngleAxisf RotAngle;
    RotAngle.fromRotationMatrix(Rcl);
    mRNorm = RotAngle.angle() * 180 / 3.1415926;
    mTNorm = tcl.norm();
    // cout << "RNorm: " << RNorm << " TNorm: " << TNorm << endl;

    int th; // 设置匹配过程中的搜索半径
    if(mSensor==System::STEREO)
        // th = 7 + 2*max(int(mRNorm-0.5), 0);
        th = 7;
    else th = 15;

    // 这里的SearchByProjection3目的是对上一帧中包含的MP进行跟踪，从而增加当前帧的MP，返回匹配点数
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR);

    // If few matches, uses a wider window search
    // window的大小应该是只跟运动有关？运动剧烈的情况下需要使用更大的window去搜索，如果本来就很难匹配，那么提升window大小也没啥用
    if(nmatches<20){
    ...
    if(nmatches<20)
    {
        ...
        if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            return true; // 带IMU应该还是能跟上
        else
            return false; // 纯视觉认为是丢失了
    }

    // Optimize frame pose with all matches
    // 利用3D-2D投影关系，优化当前帧位姿
    // mLastFrame在完成匹配之后其实没啥用了，下面用的都是mCurrentFrame
    Optimizer::PoseOptimization(&mCurrentFrame);
    ...
    if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
        ...
        return nmatchesMap>=10; // 超过10个点就认为是跟踪成功
}

// 搜索局部关键帧和局部地图点，和当前帧进行投影匹配，得到更多MP优化位姿
bool Tracking::TrackLocalMap()
{
    ...
    // 更新当前帧的地图点被观测的程度，并且统计跟踪局部地图后匹配数目
    for(int i=0; i<mCurrentFrame.N; i++){
        if(mCurrentFrame.mvpMapPoints[i]){
            // 由于当前帧的地图点可以被当前帧观测到，其被观测统计量加一
            if(!mCurrentFrame.mvbOutlier[i]){
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                if(!mbOnlyTracking){
                    // 如果该地图点被相机观测数目nObs>0，匹配内点数加一
            ...
            else if(mSensor==System::STEREO) // 为什么双目直接清除了
                mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
        }
    }
...
// 检查地图点的投影是否处于高熵区域，1：高熵; 0：低熵；-1：无效，这个地方还没有进行投影匹配
int Tracking::CheckMapPointEntropy(const float thEntropy, const int i){
    int stride = sqrt(mBlockNum);
    int lx = 640 / stride;
    int ly = 480 / stride;
    const float z = mCurrentFrame.mvDepth[i];
    if(z > 0){
        unique_lock<mutex> lock(mMutexUpdateEntropy);
        float u = mCurrentFrame.mvKeysUn[i].pt.x;
        float v = mCurrentFrame.mvKeysUn[i].pt.y;
        int idx = (int(u / lx) * stride) + int(v / ly);
        // mvBlockEntropy没有东西
        if(mvBlockEntropy[idx] >= thEntropy)
            return 1;
        else
            return 0;
    }
...
// 计算每个小块内的熵
void Tracking::CalculateBlockEntropy(const float th)
{
    unique_lock<mutex> lock(mMutexUpdateEntropy);
    mvBlockEntropy.clear(); // 存储块的熵
    mvBlockEntropy.resize(mBlockNum);
    cv::Mat img = mImGray;
    cv::Rect m_select;
    float entropy;
    mLowEntBlocks = 0;
    ...
    int stride = sqrt(mBlockNum);
    int lx = img.rows / stride;
    int ly = img.cols / stride;
    int pixel_x = 0; int pixel_y = 0;

    for (int i = 0; i < mBlockNum; i++){
        int x1 = pixel_x; int x2 = pixel_x + stride;
        int y1 = pixel_y; int y2 = pixel_y + stride;

        m_select = cv::Rect(x1, y1, lx, ly);
        cv::Mat ROI = img(m_select);
        entropy = Entropy(ROI);
        mvBlockEntropy[i] = entropy;
...
int SearchNearKP(const int d){
    vector<cv::KeyPoint> MatchedKP;
}

vector<cv::KeyPoint> Tracking::GetORBPoints()
{
    // unique_lock<mutex> lock(mMutexORBpoints);   
    return mpORBextractorLeft->mvKeyPoints;
}

// 计算灰度图像熵，这个快的有点离谱，找一下直方图是怎么计算图像熵的
float Tracking::Entropy(cv::Mat image)
{
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 维护一个窗口，检查相机是否是低纹理向高纹理运动
    // auto startTime = chrono::system_clock::now();
    int windowSize = 10; // 规定滑动窗口
    float entropy = Entropy(mImGray);
    mqEntropy.push(entropy);
    float EntDiff = 0.0f; // 图像熵的差值

    // 只会在初始化的时候执行，后面全是滑动窗口
    if(mqEntropy.size() <= windowSize){
        windowSum += entropy;
        avgEnt = windowSum / mqEntropy.size();
    }
    
    else{
        avgEnt += (entropy - mqEntropy.front()) / windowSize;
        EntDiff = entropy - avgEnt; // 除了判定低纹理转向高纹理，应该还可以做更多的事情
        // cout << "EntDiff: " << EntDiff << endl;
        mqEntropy.pop();
    }
    // auto endTime = chrono::system_clock::now();
    // auto duration = chrono::duration_cast<chrono::microseconds>(endTime-startTime).count();
    // cout << "Time: " << duration * 0.001 << " ms " << endl;
    ...
    float thHighTexture = 0.11f;

    // Condition 1a: More than "MaxFrames" have passed from last keyframe insertion
    const bool c1a = mCurrentFrame.mnId>=mnLastKeyFrameId+mMaxFrames;
    // Condition 1b: More than "MinFrames" have passed and Local Mapping is idle ,原来是0,换成3了
    ...
    // Condition 2a: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2a = (((mnMatchesInliers < nRefMatches*thRefRatio || bNeedToInsertClose)) && mnMatchesInliers>15);
    // Condition 2b: Camera moving from low-texture scene to high-texture scene
    // const bool c2b = ((mnMatchesInliers > nRefMatches * thHighTexture) && mnMatchesInliers > 15);
    const bool c2b = ((EntDiff >= thHighTexture) && mnMatchesInliers > 15);

    // cout << "c2a: " << c2a << " c2b: " << c2b << endl;

    // 主要是c2出问题了，而c2出问题的原因是 nRefMatches 太少，导致乘以thRefRatio之后变少， 
    ...
    if(((c1a||c1b||c1c) && (c2a || c2b))|| c3 ||c4)
    // if(((c1a||c1b||c1c) && c2a)||c3 ||c4)
    {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // 双目的时候才会进入，和UpdateLastFrame相同
    if(mSensor!=System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        // 看一下这个地方，根据 Tcw计算mRcw、mTcw、mRwc、mOw，应该是这里出问题了,当关键点都聚集在图像的某个角落时，求解会变得困难
        mCurrentFrame.UpdatePoseMatrices();
        // cout << "updatePoseMatrics" << endl;
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                // 如果这个点在上一帧中没有对应的地图点，或者创建之后没有被观测到，那么就生成一个临时点
                MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
                if(!pMP)
                    bCreateNew = true;
                else if(pMP->Observations()<1)
                ...
                if(bCreateNew)
                {
                    ...
                    // cout << "Create New MP by KF" << endl;

                    // 相比于临时地图点，下面是永久地图点需要做的更多处理，即三部曲
...
void Tracking::SearchLocalPoints()
{
    ...
    // 遍历当前帧的地图点，标记这些地图点不再参与后面的投影搜索匹配
    ...
    // 判断所有的局部地图点中除了当前帧地图点外的点，是否在当前帧的视野范围内
    ...
    // Eigen::AngleAxisf RotAngle;
    // RotAngle.fromRotationMatrix(Rcl);
    // float RNorm = RotAngle.angle() * 180 / 3.1415926;
    // float TNorm = tcl.norm();
    // cout << "RNorm: " << RNorm << " TNorm: " << TNorm << endl;

    if(nToMatch>0){
        ORBmatcher matcher(0.8); // 原来是0.8
        // int th = 1 + int(RNorm - 0.5);
        // int th = 1 + max(int(mRNorm-0.5), 0);
        // if(mTNorm >= 0.3){
        //     th += 1;
        // }
        int th =  1;
        ...
        // 使用SearchByProjection将局部地图点投影到当前帧进行跟踪
...
void Tracking::UpdateLocalPoints()
{
    ...
    for(vector<KeyFrame*>::const_reverse_iterator itKF=mvpLocalKeyFrames.rbegin(), itEndKF=mvpLocalKeyFrames.rend(); itKF!=itEndKF; ++itKF)
    {
        ...
        // 把局部关键帧的地图点添加到 mvpLocalMapPoints
        for(vector<MapPoint*>::const_iterator itMP=vpMPs.begin(), itEndMP=vpMPs.end(); itMP!=itEndMP; itMP++)
        {
            MapPoint* pMP = *itMP;
            if(!pMP)
                continue;
            // 记录当前帧的id，防止重复添加局部地图点
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // 遍历当前帧的参考关键帧，记录所有能观测到的当前帧地图点的关键帧
        ...
        for(int i=0; i<mCurrentFrame.N; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    // 由于一个MP可以被多个关键帧观测到，因此对于每一次观测，都对观测到这个MP的关键帧进行累计
                    for(map<KeyFrame*,tuple<int,int>>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
                        // it->first是MP看到的关键帧id，keyframeCounter实际上是map容器，表示该KF看到了多少关键帧
    ...
    mvpLocalKeyFrames.reserve(3*keyframeCounter.size()); // 申请三杯内存

    // All keyframes that observe a map point are included in the local map. Also check which keyframe shares most points
    // 类型1：能观测到当前帧MP的关键帧作为局部关键帧（一级共视关键帧）
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        // 拿10个二级共视关键帧出来，不足10个就全拿了
        const vector<KeyFrame*> vNeighs = pKF->GetBestCovisibilityKeyFrames(10);

        // vNeights是按照共视程度从大到小排列的
...
vector<KeyFrame*> Tracking::GetLocalKeyFrames(){
