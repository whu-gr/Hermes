...
void System::SaveTrajectoryICL(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        f << cnt << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        cnt++;
    }
    f.close();
}

void System::SaveTrajectoryTartanAir(const string &filename)
{
    cout << "Saving Tartanair camera trajectory to " << filename << " ..." << endl;
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    // 获取地图集中所有的关键真，并且根据id排序
    vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);

    // 转换所有的关键帧，使第一个关键帧位于原点
    // 闭环之后第一个关键帧可能不在原点
    Sophus::SE3f Two = vpKFs[0]->GetPoseInverse();
    ...
    // 对于每一帧都有一个参考关键帧（lRit），时间戳（lT）和跟踪失败时为真的标志（lbL）
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        KeyFrame* pKF = *lRit;
        Sophus::SE3f Trw;
        // 如果参考关键真被标记为坏（检查冗余时被剔除了）那么就利用其父关键帧，记录父关键帧到原参考关键帧的位姿变换
        while(pKF->isBad())
        {
            Trw = Trw * pKF->mTcp; // 更新关键帧变换矩阵的初始值
            pKF = pKF->GetParent(); // 更新到原关键帧的父关键帧
        }

        // 最终得到的是参考关键相对世界坐标系的变换
        Trw = Trw * pKF->GetPose() * Two;

        // 在此基础上得到相机当前帧相对于世界坐标系的变换，*lit = Tcr
        Sophus::SE3f Tcw = (*lit) * Trw;
        Sophus::SE3f Twc = Tcw.inverse();
...
void System::SaveKeyFrameTimeStamp(const string &filename)
{
    cout << endl << "Saving keyframe timestamp to " << filename << " ..." << endl;
    vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    ofstream f;
    f.open(filename.c_str());
...
void System::SaveKFsMPs(const string &filename)
{
    cout << endl << "Saving MPs & KFs to " << filename << " ..." << endl;
    ...
    f.open(filename.c_str());
    f << fixed;
    f << "Maps: " << nMaps << " KFs: " << nKFs << " MPs: " << nMPs << endl;
    f.close();
}
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        else
        {
            ...
            f << setprecision(6) << pKF->mTimeStamp << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
