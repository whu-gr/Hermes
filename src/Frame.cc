...
void Frame::UpdatePoseMatrices()
{
    ...
    mOw = Twc.translation();  // 当前相机光心在世界坐标系下的坐标
...
// 找到在 以x,y为中心，半径为r的圆形内且金字塔层级在[minLevel, maxLevel]的特征点，相当于指定搜索范围
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel, const bool bRight) const
{
    // 储存与帧1特征点所在网格对应的帧2网格图中所在网格内所有特征点的索引
    // 也就是帧1中的某个特征点对应的帧2中一个ws×ws大小的网格中的特征点
    ...
    // 计算半径为r的圆左右上下边界所在的网格列和行的id
    const int nMinCellX = max(0,(int)floor((x-mnMinX-factorX)*mfGridElementWidthInv));

    // corner case: 最终求得的圆左/右边界所在网格列超过设定上限，说明计算出错，找不到符合的
    ...
    // 遍历圆形区域内的所有网格，寻找满足条件的候选特征点，将index放在输出里面
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            // 获取这个网格内的所有特征点在Frame::mvKeyUn中的索引 mGrid是图像网格化结果 vector<vector<vector<size_t>>>
            const vector<size_t> vCell = (!bRight) ? mGrid[ix][iy] : mGridRight[ix][iy];
            // 网格中没有特征点，那么跳过这个网格继续下一个
            if(vCell.empty())
                continue;

            // 如果这个网格中有特征点，那么跳过这个网格继续下一个
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                {
                    // 表示从金字塔哪一层提取的数据
                    // 保证金字塔层级minLevel和maxLevel之间，不是的话跳过
                ...
                // 通过检查，计算候选特征点到圆中心的距离，检查是否在这个圆形区域中
                const float distx = kpUn.pt.x-x;
                const float disty = kpUn.pt.y-y;

                // 如果x、y方向距离都在指定半径内，储存index为候选特征点
