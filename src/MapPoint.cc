...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mnOriginMapId(pMap->GetId()), isInLowEntropy(false), mDensity(0), isDense(false), isChecked(false)
{
    SetWorldPos(Pos);
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    isInLowEntropy(false), mDensity(0), isDense(false), isChecked(false)
{
    SetWorldPos(Pos);
...
int MapPoint::GetDensity(){
    return mDensity;
}

void MapPoint::AddDensity(){
    mDensity++;
}

void MapPoint::SetDense(){
    isDense = true;
}

void MapPoint::SetChecked(){
    isChecked = true;
}

// 增加地图点的观测关系，输入是关键帧和其对应的索引
// 函数的作用是判断此关键帧是否已经在观测关系中，如果是，就不会添加了，如果不是，记录次关键帧以及MP的索引
void MapPoint::AddObservation(KeyFrame* pKF, int idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    tuple<int,int> indexes;

    // 容器 map<KeyFrame*,std::tuple<int,int> > mObservations 存放观测关系<观测到MP的KF，MP在该KF中索引>
    if(mObservations.count(pKF)){
        indexes = mObservations[pKF];
    }
    else{
        indexes = tuple<int,int>(-1,-1); // 没有观测值就重新定义一个
    }
    ...
    if(pKF -> NLeft != -1 && idx >= pKF -> NLeft){
        ...
        get<0>(indexes) = idx; // //indexes[0]存储当前地图点对应的关键帧的特征/地图点ID
    }

    mObservations[pKF]=indexes;

    // 单目 + 1，双目 + 2，nObs是MapPoint里面的一个成员属性
