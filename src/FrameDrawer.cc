...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    vector<bool> vbDense, vbLowEntropy;
    ...
    // cv::drawKeypoints(im, ORBkp, im);

    cv::Scalar standardColor(0,255,0); // 绿
    cv::Scalar odometryColor(255,0,0); // 蓝
    cv::Scalar denseColor(0,255,255); // 黄
    cv::Scalar LowEntColor(255,255,255); // 白
    cv::Scalar BothColor(0,0,255); // 红
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i] || vbMap[i])
            {
                ...
                mvMatchedKPs.push_back(vCurrentKeys[i]);

                // This is a match to a MapPoint in the map
                if(vbMap[i])
                {
                    if(vbDense[i] && vbLowEntropy[i]){
                        cv::rectangle(im,pt1,pt2,BothColor);
                        cv::circle(im,point,2,BothColor,-1);
                    }
                    else if(vbLowEntropy[i] && !vbDense[i]){
                        cv::rectangle(im,pt1,pt2,LowEntColor);
                        cv::circle(im,point,2,LowEntColor,-1);
                    }
                    else if(!vbLowEntropy[i] && vbDense[i]){
                        cv::rectangle(im,pt1,pt2,denseColor);
                        cv::circle(im,point,2,denseColor,-1);
                    }
                    else{
...
cv::Mat FrameDrawer::DrawORBPoints(cv::Mat &im, Tracking *pTracker)
{
    vector<cv::KeyPoint> vORBPoints = pTracker->GetORBPoints();
    // cout << "ORB Points size: " << vORBPoints.size() << endl;
    cv::drawKeypoints(im, vORBPoints, im);
    return im;
}

vector<cv::KeyPoint> FrameDrawer::Keys()
{
    // unique_lock<mutex> lock(mMutexMatchedKPs);
    return mvMatchedKPs;
}
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i + Nleft] || vbMap[i + Nleft])
            {
                ...
                // This is a match to a MapPoint in the map, green
                if(vbMap[i + Nleft])
                {
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    mvbDense = vector<bool>(N, false);
    mvbLowEntropy = vector<bool>(N,false);
        ...
        for(int i=0;i<N;i++)
        {
            ...
            if(pMP)
            {
                if(!pTracker->mCurrentFrame.mvbOutlier[i])
                {
                    ...
                    if(pMP->isDense)
                        mvbDense[i] = true;
                    if(pMP->isInLowEntropy)
                        mvbLowEntropy[i] = true;
